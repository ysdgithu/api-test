...
3. **脚本运行**: 执行测试脚本
4. **报告输出**: AI生成测试报告（.md格式），保存到 `{project_root}/{module_name}/test_report.md`，报告中仅记录问题，不尝试修复
...
3. **脚本运行**: 执行已有测试脚本
4. **报告输出**: AI更新测试报告（.md格式），保存到 `{project_root}/{module_name}/test_report.md`，报告中仅记录问题，不尝试修复
5. **工作流结束**: AI告知用户测试报告路径
